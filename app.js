...
v = null;
if (["labor_hours", "labor_rate", "markup_pct", "min_qty", "qty", "sell_price", "service_interval", "total", "unit_price", "price", "cost"].includes(sk) && v === "")
v = null;
out[sk] = v;
}
return out;
...
if (error)
return false;
return (data === null || data === void 0 ? void 0 : data.role) === "tecnico";
});
}
function useMedia(q) {
...
<div class="header">
<div>
${company.logo ? `<img src="${company.logo}" class="header-logo"/>` : ""}
<h1>${company.name || "Report"}</h1>
<div class="sub">${company.subtitle || ""}</div>
</div>
<div class="right">
<div style="font-size:9px;opacity:.7;margin-bottom:4px">REPORT PARCO MACCHINE</div>
...
"STICKER QR: dopo aver salvato una verifica, l'app ti propone di stampare uno sticker adesivo con QR da applicare sull'apparecchio. Lo trovi anche col pulsante 🏷 nelle liste. Da scansionare con qualsiasi telefono; con il cloud attivo il QR apre il report online."
]
},
{
...
React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 14 } },
React.createElement(Section, { icon: "\uD83C\uDFE2", title: "Dati azienda" },
!comp.name && (React.createElement("div", { style: { background: "#f59e0b15", border: "1px solid #f59e0b44", borderRadius: 8, padding: "10px 13px", marginBottom: 14, fontSize: 12, color: "#f59e0b", lineHeight: 1.5 } }, "\u26A0 Inserisci il nome della tua azienda \u2014 apparir\u00E0 su tutti i PDF (rapporti, verifiche, preventivi).")),
React.createElement(Grid, { cols: "1fr 1fr" },
...
React.createElement(Btn, { sm: true, onClick: () => { onUpdateCompany(comp); alert("Dati azienda salvati."); } }, "Salva dati azienda"))),
...
const nextDate = (asset === null || asset === void 0 ? void 0 : asset.nextService) ? ("Prossima: " + asset.nextService) : "";
...
${nextDate ? `<div class="row">${nextDate}</div>` : ""}
...
" contiene le info della verifica (macchina, data, esito). Per far aprire il report online dal QR, attiva la sincronizzazione Supabase nelle Impostazioni.")),
React.createElement("div", { style: FORM_FOOTER },
React.createElement("button", { onClick: onClose, style: FORM_BTN_GHOST }, "Chiudi"),
...
React.createElement("span", { style: { fontSize: 10, color: "#334155", fontFamily: "monospace", letterSpacing: .5 } }, "v0.88"),
...
], rows: customers.map(c => (Object.assign(Object.assign({}, c), { nApparecchi: assets.filter(a => a.customerId === c.id).length }))), onEdit: row => setModal({ type: "customer", data: customers.find(c => c.id === row.id) }), onDelete: id => delCustomer(id), actions: row => (React.createElement(React.Fragment, null,
...
React.useEffect(() => {
if (isDemo) {
setCurrentUser({ id: 'demo-user', email: 'demo@medtrace' });
setAuthReady(true);
return;
}
let tries = 0;
