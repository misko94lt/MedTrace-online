...
const STATUS_COLOR = { "operativo": "#22c55e", "in manutenzione": "#f59e0b", "fuori servizio": "#ef4444", "da accettare": "#818cf8", "aperto": "#2DD4BF", "in corso": "#f59e0b", "in attesa ricambi": "#f59e0b", "in attesa cliente": "#a855f7", "in attesa accesso": "#38bdf8", "chiuso": "#22c55e", "in attesa": "#94a3b8", "confermato": "#2DD4BF", "spedito": "#a855f7", "ricevuto": "#22c55e", "annullato": "#ef4444", "emessa": "#2DD4BF", "pagata": "#22c55e", "scaduta": "#ef4444", "bozza": "#94a3b8" };
...
admin: ["dashboard", "kpi", "jobs", "richieste", "iec", "func", "agenda", "scadenze", "assets", "instruments", "customers", "parts", "invoices", "procedures", "help"],
finance: ["dashboard", "kpi", "jobs", "assets", "customers", "invoices", "procedures", "help"],
tecnico: ["dashboard", "kpi", "jobs", "richieste", "iec", "func", "agenda", "scadenze", "assets", "instruments", "customers", "parts", "procedures", "help"],
};
...
if (!_quotaAlerted) {
_quotaAlerted = true;
alert("⚠ SPAZIO PIENO — L'ULTIMA MODIFICA NON È STATA SALVATA.\n\nCosa fare:\n1) Impostazioni → Esporta backup (metti subito al sicuro i dati)\n2) Elimina qualche foto pesante dai job\n\nIn Impostazioni → \"Spazio dati locale\" vedi quanto occupano le foto.");
}
return false;
}
}
...
const mergeById = (a, b) => {
const __hasData = (rec) => { if (rec && Array.isArray(rec.measures)) { return rec.measures.some(m => m && String(m.value == null ? "" : m.value).trim() !== ""); } if (rec && rec.sections && typeof rec.sections === "object") { return Object.values(rec.sections).some(s => { if (!s) return false; const mm = s.measures && typeof s.measures === "object" ? Object.values(s.measures) : []; const ii = s.items && typeof s.items === "object" ? Object.values(s.items) : []; return mm.some(v => String(v == null ? "" : v).trim() !== "") || ii.some(v => v === true || v === false); }); } return null; };
const map = new Map();
...
const dEx = __hasData(ex), dNew = __hasData(r);
let winner;
if (dNew === true && dEx === false) winner = Object.assign({}, r, { updatedAt: new Date().toISOString() });
else if (dEx === true && dNew === false) winner = Object.assign({}, ex, { updatedAt: new Date().toISOString() });
...
function supabaseDeleteById(stateKey, id) {
return __awaiter(this, void 0, void 0, function* () {
try {
if (typeof OFFLINE_MODE !== "undefined" && OFFLINE_MODE)
return;
...
function supabasePushOne(stateKey, record) {
return __awaiter(this, void 0, void 0, function* () {
try {
if (typeof OFFLINE_MODE !== "undefined" && OFFLINE_MODE)
return false;
...
const __nameEl = React.createElement("span", { style: { fontSize: 12.5, color: fail ? "#fecaca" : "#e2e8f0", fontWeight: fail ? 700 : 500, textDecoration: isNA ? "line-through" : "none", overflowWrap: "anywhere", lineHeight: 1.3, minWidth: 0 } }, m.name);
const __expEl = React.createElement("span", { style: { fontSize: 10, color: "#94a3b8", fontFamily: "monospace" } }, m.expected || m.limit || "");
const __inputEl = isNA ? React.createElement("span", { style: { fontSize: 10, color: "#94a3b8", fontStyle: "italic", textAlign: "center" } }, "N/A") : React.createElement("input", { type: "number", step: "any", inputMode: "decimal", value: raw, onChange: e => setMeasure(secId, m.id, e.target.value), placeholder: "\u2014", style: { background: fail ? "#5b1414" : "#1c1c24", border: "2px solid " + (fail ? "#f87171" : "#33394a"), borderRadius: 5, padding: "6px 8px", color: fail ? "#ffe4e4" : "#ffffff", fontSize: 14, outline: "none", fontFamily: "monospace", fontWeight: 800, boxShadow: fail ? "0 0 0 1px #ef4444" : "none", width: isMobile ? 96 : undefined, minWidth: 0 } });
const __unitEl = React.createElement("span", { style: { fontWeight: 800, fontSize: 14, textAlign: "center", color: pass === null ? "#64748b" : pass ? "#34d399" : "#f87171", minWidth: isMobile ? 22 : undefined } }, m.unit);
const __naBtn = React.createElement("button", { title: isNA ? "Ripristina misura" : "Marca come Non Applicabile", onClick: () => setMeasure(secId, m.id, isNA ? "" : "na"), style: { background: isNA ? "#64748b33" : "#1c1c24", border: "1px solid " + (isNA ? "#64748b66" : "#33394a"), color: isNA ? "#cbd5e1" : "#94a3b8", borderRadius: 5, padding: isMobile ? "5px 10px" : "4px 4px", cursor: "pointer", fontSize: isMobile ? 11 : 9, fontWeight: 700, whiteSpace: "nowrap", flexShrink: 0, touchAction: "manipulation", WebkitTapHighlightColor: "transparent" } }, "N/A");
return (React.createElement("div", { style: { marginBottom: fail ? 8 : 6 } },
(isMobile ? React.createElement("div", { style: { background: fail ? "#3a0f0f" : "#0D0D12", borderRadius: 6, padding: "8px 10px", opacity: isNA ? 0.55 : 1, border: fail ? "2px solid #ef4444" : "1px solid #1e2a3a" } }, React.createElement("div", { style: { marginBottom: 7 } }, __nameEl), React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" } }, React.createElement("span", { style: { fontSize: 10, color: "#94a3b8", fontFamily: "monospace", flex: "1 1 60px", minWidth: 0 } }, m.expected || m.limit || ""), __inputEl, __unitEl, __naBtn)) : React.createElement("div", { style: { display: "grid", gridTemplateColumns: "minmax(0,1fr) 90px 60px 38px 30px", gap: 6, alignItems: "center", background: fail ? "#3a0f0f" : "#0D0D12", borderRadius: 6, padding: "6px 8px", opacity: isNA ? 0.55 : 1, border: fail ? "2px solid #ef4444" : "1px solid #1e2a3a" } }, __nameEl, __expEl, __inputEl, __unitEl, __naBtn)),
...
const C = { box: "#1a2030", boxStroke: "#3a4560", wireRef: "#ef4444", wireProbe: "#cbd5e1", wirePA: "#a855f7", txt: "#e2e8f0", sub: "#94a3b8" };
const Box = ({ x, y, w, h, label }) => (React.createElement("g", null,
React.createElement("rect", { x: x, y: y, width: w, height: h, rx: 9, fill: C.box, stroke: C.boxStroke, strokeWidth: 1.5 }),
React.createElement("text", { x: x + w / 2, y: y + h / 2 + 4, textAnchor: "middle", fill: C.txt, fontSize: 12, fontWeight: 600, fontFamily: "system-ui" }, label)));
const Dot = ({ x, y, c }) => React.createElement("circle", { cx: x, cy: y, r: 5, fill: c, stroke: "#0a0a0f", strokeWidth: 1 });
const Leg = ({ x, y, c, t }) => (React.createElement("g", null,
React.createElement("line", { x1: x, y1: y, x2: x + 18, y2: y, stroke: c, strokeWidth: 2.5 }),
React.createElement("text", { x: x + 24, y: y + 4, fill: C.sub, fontSize: 10.5, fontFamily: "system-ui" }, t)));
if (kind === "pe_mobile")
return (React.createElement("svg", { viewBox: "0 0 340 215", style: { width: "100%", maxWidth: 340, height: "auto" } },
React.createElement(Box, { x: 20, y: 30, w: 130, h: 80, label: "Apparecchio" }),
React.createElement(Box, { x: 210, y: 30, w: 110, h: 80, label: "Analizzatore" }),
React.createElement("path", { d: "M150 95 H210", stroke: C.wireRef, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 180, y: 88, textAnchor: "middle", fill: C.sub, fontSize: 10, fontFamily: "system-ui" }, "spina \u2192 analizzatore"),
React.createElement(Dot, { x: 150, y: 50, c: C.wireProbe }),
React.createElement("path", { d: "M210 50 H150", stroke: C.wireProbe, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 150, y: 22, textAnchor: "middle", fill: C.sub, fontSize: 10.5, fontFamily: "system-ui" }, "equipotenziale / parti metalliche"),
React.createElement(Leg, { x: 20, y: 185, c: C.wireRef, t: "spina dell'apparecchio nell'analizzatore" }),
React.createElement(Leg, { x: 20, y: 203, c: C.wireProbe, t: "sonda mobile sull'equipotenziale macchina" })));
if (kind === "pe_fixed")
return (React.createElement("svg", { viewBox: "0 0 340 230", style: { width: "100%", maxWidth: 340, height: "auto" } },
React.createElement(Box, { x: 20, y: 30, w: 130, h: 80, label: "Apparecchio (fisso)" }),
React.createElement(Box, { x: 210, y: 30, w: 110, h: 80, label: "Analizzatore" }),
React.createElement(Dot, { x: 45, y: 155, c: C.wireRef }),
React.createElement("path", { d: "M255 110 V185 H45 V155", stroke: C.wireRef, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 55, y: 159, fill: C.sub, fontSize: 10, fontFamily: "system-ui" }, "PE impianto (quadro)"),
React.createElement("text", { x: 258, y: 128, textAnchor: "middle", fill: C.sub, fontSize: 9.5, fontFamily: "system-ui" }, "aux ground"),
React.createElement(Dot, { x: 150, y: 50, c: C.wireProbe }),
React.createElement("path", { d: "M210 50 H150", stroke: C.wireProbe, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 150, y: 22, textAnchor: "middle", fill: C.sub, fontSize: 10.5, fontFamily: "system-ui" }, "equipotenziale macchina"),
React.createElement(Leg, { x: 20, y: 208, c: C.wireRef, t: "cavo aux ground → PE impianto" }),
React.createElement(Leg, { x: 20, y: 225, c: C.wireProbe, t: "sonda → equipotenziale macchina" })));
if (kind === "encl")
return (React.createElement("svg", { viewBox: "0 0 340 215", style: { width: "100%", maxWidth: 340, height: "auto" } },
React.createElement(Box, { x: 20, y: 30, w: 140, h: 85, label: "Apparecchio" }),
React.createElement("text", { x: 90, y: 102, textAnchor: "middle", fill: C.sub, fontSize: 9.5, fontFamily: "system-ui" }, "(acceso)"),
React.createElement(Box, { x: 215, y: 30, w: 105, h: 85, label: "Analizzatore" }),
React.createElement("path", { d: "M160 95 H215", stroke: C.wireRef, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 187, y: 88, textAnchor: "middle", fill: C.sub, fontSize: 9.5, fontFamily: "system-ui" }, "spina MD"),
React.createElement(Dot, { x: 160, y: 50, c: C.wireProbe }),
React.createElement("path", { d: "M215 50 H160", stroke: C.wireProbe, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 160, y: 22, textAnchor: "middle", fill: C.sub, fontSize: 10.5, fontFamily: "system-ui" }, "involucro (parti non a terra)"),
React.createElement(Leg, { x: 20, y: 190, c: C.wireRef, t: "spina del MD nell'analizzatore" }),
React.createElement(Leg, { x: 20, y: 207, c: C.wireProbe, t: "sonda sull'involucro" })));
if (kind === "ins")
return (React.createElement("svg", { viewBox: "0 0 340 195", style: { width: "100%", maxWidth: 340, height: "auto" } },
React.createElement(Box, { x: 20, y: 35, w: 140, h: 80, label: "Apparecchio" }),
React.createElement("text", { x: 90, y: 105, textAnchor: "middle", fill: C.sub, fontSize: 9.5, fontFamily: "system-ui" }, "(SPENTO, scollegato)"),
React.createElement(Box, { x: 215, y: 35, w: 105, h: 80, label: "Analizzatore" }),
React.createElement("path", { d: "M160 60 H215", stroke: C.wireRef, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 187, y: 53, textAnchor: "middle", fill: C.sub, fontSize: 9.5, fontFamily: "system-ui" }, "500 Vdc \u2192 rete"),
React.createElement("path", { d: "M160 90 H215", stroke: C.wireProbe, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 185, y: 104, textAnchor: "middle", fill: C.sub, fontSize: 9.5, fontFamily: "system-ui" }, "parti access. / terra"),
React.createElement(Leg, { x: 20, y: 172, c: C.wireRef, t: "500 Vdc applicati ai conduttori di rete" })));
if (kind === "encl_fixed")
return (React.createElement("svg", { viewBox: "0 0 340 215", style: { width: "100%", maxWidth: 340, height: "auto" } },
React.createElement(Box, { x: 20, y: 30, w: 140, h: 85, label: "Apparecchio (fisso)" }),
React.createElement("text", { x: 90, y: 102, textAnchor: "middle", fill: C.sub, fontSize: 9.5, fontFamily: "system-ui" }, "(acceso, su sua rete)"),
React.createElement(Box, { x: 215, y: 30, w: 105, h: 85, label: "Analizzatore" }),
React.createElement(Dot, { x: 160, y: 50, c: C.wireRef }),
React.createElement("path", { d: "M215 50 H160", stroke: C.wireRef, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 150, y: 22, textAnchor: "middle", fill: C.sub, fontSize: 10, fontFamily: "system-ui" }, "ROSSO \u2192 involucro macchina"),
React.createElement(Dot, { x: 45, y: 155, c: C.wireProbe }),
React.createElement("path", { d: "M265 115 V185 H45 V155", stroke: C.wireProbe, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 55, y: 159, fill: C.sub, fontSize: 10, fontFamily: "system-ui" }, "NERO \u2192 nodo equipotenziale"),
React.createElement(Leg, { x: 20, y: 205, c: C.wireRef, t: "rosso sulla parte da misurare" })));
if (kind === "pat_fixed")
return (React.createElement("svg", { viewBox: "0 0 340 215", style: { width: "100%", maxWidth: 340, height: "auto" } },
React.createElement(Box, { x: 20, y: 30, w: 140, h: 85, label: "Apparecchio (fisso)" }),
React.createElement(Box, { x: 215, y: 30, w: 105, h: 85, label: "Analizzatore" }),
React.createElement(Dot, { x: 160, y: 48, c: C.wireRef }),
React.createElement(Dot, { x: 160, y: 66, c: C.wireRef }),
React.createElement("path", { d: "M215 48 H160", stroke: C.wireRef, strokeWidth: 2.5, fill: "none" }),
React.createElement("path", { d: "M215 66 H160", stroke: C.wireRef, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 150, y: 22, textAnchor: "middle", fill: C.sub, fontSize: 10, fontFamily: "system-ui" }, "ROSSO \u2192 parte applicata"),
React.createElement(Dot, { x: 45, y: 155, c: C.wireProbe }),
React.createElement("path", { d: "M265 115 V185 H45 V155", stroke: C.wireProbe, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 55, y: 159, fill: C.sub, fontSize: 10, fontFamily: "system-ui" }, "NERO \u2192 nodo equipotenziale"),
React.createElement(Leg, { x: 20, y: 205, c: C.wireRef, t: "rosso sui terminali P.A. (una funzione)" })));
return (React.createElement("svg", { viewBox: "0 0 340 235", style: { width: "100%", maxWidth: 340, height: "auto" } },
React.createElement(Box, { x: 20, y: 35, w: 130, h: 95, label: "Apparecchio" }),
React.createElement(Box, { x: 225, y: 35, w: 95, h: 95, label: "Analizzatore" }),
React.createElement("path", { d: "M150 115 H225", stroke: C.wireRef, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 187, y: 108, textAnchor: "middle", fill: C.sub, fontSize: 9.5, fontFamily: "system-ui" }, "spina MD"),
React.createElement(Dot, { x: 150, y: 52, c: C.wirePA }),
React.createElement(Dot, { x: 150, y: 72, c: C.wirePA }),
React.createElement(Dot, { x: 150, y: 92, c: C.wirePA }),
React.createElement("path", { d: "M150 52 Q195 52 225 62", stroke: C.wirePA, strokeWidth: 2.5, fill: "none" }),
React.createElement("path", { d: "M150 72 H225", stroke: C.wirePA, strokeWidth: 2.5, fill: "none" }),
React.createElement("path", { d: "M150 92 Q195 92 225 80", stroke: C.wirePA, strokeWidth: 2.5, fill: "none" }),
React.createElement("text", { x: 150, y: 26, textAnchor: "middle", fill: C.sub, fontSize: 10, fontFamily: "system-ui" }, "terminali stessa P.A. (es. elettrodi ECG)"),
React.createElement(Leg, { x: 20, y: 208, c: C.wireRef, t: "spina del MD nell'analizzatore" }),
React.createElement(Leg, { x: 20, y: 225, c: C.wirePA, t: "terminali stessa funzione → morsetto P.A." })));
}
...
return Object.assign(Object.assign({}, init), { measures: getMeasures(init.norm || "62353", "I", init.patientType || "BF", init.leakageMethod || "diretto", init.sfc !== false, init.fixedInstall) });
return init;
});
const [errors, setErrors] = React.useState({});
const sv = k => e => setF(x => (Object.assign(Object.assign({}, x), { [k]: e.target.value })));
...
React.createElement("span", { style: { width: 42, height: 24, borderRadius: 999, background: f.fixedInstall ? "#2DD4BF" : "#2a3040", position: "relative", transition: "background .15s", flexShrink: 0, display: "inline-block" } },
...
React.createElement("span", { style: { fontSize: 13, fontWeight: 600, color: f.fixedInstall ? "#5EEAD4" : "#e2e8f0" } },
...
isMobile ? (React.createElement("div", { style: { marginBottom: 8, background: "#0D0D12", borderRadius: 8, padding: "10px 12px" } },
React.createElement("div", { style: { display: "flex", alignItems: "flex-start", gap: 6, marginBottom: 8 } },
React.createElement("span", { style: { fontSize: 12.5, color: "#cbd5e1", fontWeight: 600, flex: 1, minWidth: 0, overflowWrap: "anywhere", lineHeight: 1.3 } }, m.name),
tutorialFor(m.id, f.fixedInstall) && React.createElement("span", { onClick: () => setTutorialOpen(tutorialOpen === m.id ? null : m.id), title: "Come mi collego", style: { cursor: "pointer", color: "#2DD4BF", border: "1px solid #2DD4BF66", borderRadius: "50%", width: 20, height: 20, minWidth: 20, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0 } }, "i")),
React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", opacity: m.na ? .5 : 1 } },
React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5 } },
React.createElement("span", { style: { fontSize: 10, color: "#475569", textTransform: "uppercase" } }, "Limite"),
m.editableLimit ? (React.createElement("input", { type: "text", disabled: m.na, value: m.limit, onChange: e => setF(x => (Object.assign(Object.assign({}, x), { measures: x.measures.map(mm => mm.id === m.id ? Object.assign(Object.assign({}, mm), { limit: e.target.value, limitVal: parseFloat(String(e.target.value).replace(/[^0-9.]/g, "")) || mm.limitVal }) : mm) }))), title: "Limite modificabile", style: { background: "#1a1f2e", border: "1px solid #2DD4BF44", borderRadius: 5, padding: "5px 7px", color: "#5EEAD4", fontSize: 12, fontFamily: "monospace", width: 70, outline: "none" } })) : (React.createElement("span", { style: { fontSize: 12, color: "#94a3b8", fontFamily: "monospace" } }, m.limit))),
React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, flex: 1, minWidth: 120 } },
React.createElement("input", { type: "number", step: "0.001", disabled: m.na, value: m.na ? "" : m.value, onChange: e => setMeas(m.id, e.target.value), placeholder: m.na ? "non applicabile" : "valore", style: { background: "#16161C", border: "1px solid #2a3040", borderRadius: 6, padding: "7px 9px", color: "#e2e8f0", fontSize: 14, outline: "none", fontFamily: "monospace", flex: 1, minWidth: 0 } }),
React.createElement("span", { style: { fontSize: 12, color: "#64748b", minWidth: 24 } }, m.unit)),
m.na ? (React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: "#64748b" } }, "N/A")) : prevReport ? (React.createElement("span", { style: { fontSize: 18, fontWeight: 700, color: trend === "worse" ? "#ef4444" : trend === "better" ? "#22c55e" : "#475569" } }, trend === "worse" ? "▲" : trend === "better" ? "▼" : (ok === null ? "—" : ok ? "✓" : "✗"))) : (React.createElement("span", { style: { fontWeight: 700, fontSize: 18, color: ok === null ? "#475569" : ok ? "#22c55e" : "#ef4444" } }, ok === null ? "—" : ok ? "✓" : "✗"))),
isOptional(m.id) && React.createElement("button", { onClick: () => toggleNA(m.id), style: { marginTop: 8, background: m.na ? "#2DD4BF22" : "transparent", border: "1px solid " + (m.na ? "#2DD4BF66" : "#2a3040"), borderRadius: 6, color: m.na ? "#5EEAD4" : "#64748b", fontSize: 11, fontWeight: 700, padding: "5px 12px", cursor: "pointer" } }, m.na ? "✓ Non applicabile (tocca per misurare)" : "Segna come non applicabile (N/A)"))) : (React.createElement("div", { style: { display: "grid", gridTemplateColumns: prevReport ? "minmax(0,1fr) 70px 80px 50px 56px 44px" : "minmax(0,1fr) 90px 90px 56px 56px", gap: 8, alignItems: "center", marginBottom: 6, background: "#0D0D12", borderRadius: 6, padding: "7px 10px", opacity: m.na ? .5 : 1 } },
React.createElement("span", { style: { fontSize: 12, color: "#94a3b8", display: "flex", alignItems: "center", gap: 6, minWidth: 0 } },
...
isOptional(m.id) && React.createElement("span", { onClick: () => toggleNA(m.id), title: m.na ? "Misura segnata non applicabile — tocca per riattivare" : "Segna come non applicabile (prova opzionale)", style: { cursor: "pointer", fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 4, border: "1px solid " + (m.na ? "#2DD4BF" : "#2a3040"), color: m.na ? "#5EEAD4" : "#475569", background: m.na ? "#2DD4BF18" : "transparent", flexShrink: 0 } }, "N/A")),
m.editableLimit ? (React.createElement("input", { type: "text", disabled: m.na, value: m.limit, onChange: e => setF(x => (Object.assign(Object.assign({}, x), { measures: x.measures.map(mm => mm.id === m.id ? Object.assign(Object.assign({}, mm), { limit: e.target.value, limitVal: parseFloat(String(e.target.value).replace(/[^0-9.]/g, "")) || mm.limitVal }) : mm) }))), title: "Limite modificabile", style: { background: "#1a1f2e", border: "1px solid #2DD4BF44", borderRadius: 5, padding: "4px 6px", color: "#5EEAD4", fontSize: 11, fontFamily: "monospace", width: "100%", outline: "none" } })) : (React.createElement("span", { style: { fontSize: 11, color: "#64748b", fontFamily: "monospace" } }, m.limit)),
React.createElement("input", { type: "number", step: "0.001", disabled: m.na, value: m.na ? "" : m.value, onChange: e => setMeas(m.id, e.target.value), placeholder: m.na ? "N/A" : "—", style: { background: "#16161C", border: "1px solid #2a3040", borderRadius: 5, padding: "4px 7px", color: "#e2e8f0", fontSize: 12, outline: "none", fontFamily: "monospace", minWidth: 0 } }),
React.createElement("span", { style: { fontSize: 11, color: "#64748b" } }, m.unit),
m.na ? (React.createElement("span", { style: { fontWeight: 700, fontSize: 12, color: "#64748b", textAlign: "center" } }, "N/A")) : prevReport ? (React.createElement(React.Fragment, null,
React.createElement("span", { style: { fontSize: 11, color: "#64748b", fontFamily: "monospace", textAlign: "center" } }, prevVal !== null ? prevVal : "—"),
React.createElement("span", { style: { fontSize: 14, textAlign: "center", fontWeight: 700, color: trend === "worse" ? "#ef4444" : trend === "better" ? "#22c55e" : "#475569" }, title: trend === "worse" ? "In peggioramento" : trend === "better" ? "In miglioramento" : (ok === null ? "" : (ok ? "Conforme" : "Non conforme")) }, trend === "worse" ? "▲" : trend === "better" ? "▼" : (ok === null ? "—" : ok ? "✓" : "✗")))) : (React.createElement("span", { style: { fontWeight: 700, fontSize: 13, color: ok === null ? "#475569" : ok ? "#22c55e" : "#ef4444" } }, ok === null ? "—" : ok ? "✓" : "✗")))),
tutorialOpen === m.id && tutorialFor(m.id, f.fixedInstall) && (() => {
const tut = tutorialFor(m.id, f.fixedInstall);
return (React.createElement("div", { key: m.id + "-tut", style: { background: "#11161F", border: "1px solid #2DD4BF", borderRadius: 10, padding: "14px 16px", marginBottom: 10, marginTop: 4, boxShadow: "0 4px 14px rgba(0,0,0,.4)" } },
...
React.createElement("div", { style: { fontSize: 12, fontWeight: 600, color: "#e2e8f0", marginBottom: 10 } }, tut.titolo),
...
React.createElement("ol", { style: { margin: 0, paddingLeft: 18, fontSize: 12, color: "#cbd5e1", lineHeight: 1.6 } }, tut.passi.map((p, i) => React.createElement("li", { key: i, style: { marginBottom: 3 } }, p))),
tut.nota && React.createElement("div", { style: { fontSize: 11, color: "#f59e0b", marginTop: 8, fontStyle: "italic" } },
"\u26A0 ",
tut.nota),
React.createElement("div", { style: { fontSize: 10, color: "#475569", marginTop: 6 } }, "Testo da rifinire con la pratica reale.")));
})()));
})),
...
note: "Uscita bipolare al suo carico dedicato (~100 Ω tipico; alcuni strumenti usano 50–200 Ω — vedi manuale). Se il bipolare ha sia COAG sia CUT, compila entrambi; molti generatori hanno solo COAG bipolare.",
items: [
...
React.createElement("tr", null,
cols.map(c => (React.createElement("td", { key: c.key, style: { padding: "3px 5px", background: "#090910", borderBottom: "2px solid #2563eb" } }, c.opts ? (() => {
...
React.createElement("button", { onClick: () => { setOptSearch(""); setOpenCol(open ? null : c.key); }, title: sel.length ? sel.join(", ") : "Filtra", style: { display: "flex", alignItems: "center", gap: 4, width: "100%", background: sel.length ? "#2DD4BF18" : "#16161C", border: "1px solid " + (sel.length ? "#2DD4BF66" : "#1e2a3a"), borderRadius: 4, padding: "3px 6px", color: sel.length ? "#5EEAD4" : "#94a3b8", fontSize: 11, cursor: "pointer", outline: "none", whiteSpace: "nowrap", overflow: "hidden" } },
...
? { position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 41, maxHeight: "62vh", overflowY: "auto", background: "#16161C", borderTop: "1px solid #2DD4BF44", borderRadius: "16px 16px 0 0", boxShadow: "0 -8px 30px rgba(0,0,0,.6)", padding: "14px 14px calc(14px + env(safe-area-inset-bottom))" }
: { position: "absolute", top: "calc(100% + 3px)", left: 0, zIndex: 41, minWidth: 180, maxWidth: 260, maxHeight: 280, overflowY: "auto", background: "#16161C", border: "1px solid #2DD4BF44", borderRadius: 8, boxShadow: "0 8px 24px rgba(0,0,0,.5)", padding: 6 } },
isMobileTbl && React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 } },
React.createElement("div", { style: { fontSize: 14, fontWeight: 800, color: "#e2e8f0" } }, c.label),
React.createElement("button", { onClick: () => setOpenCol(null), style: { background: "none", border: "none", color: "#94a3b8", fontSize: 22, cursor: "pointer", lineHeight: 1, padding: "0 4px" } }, "\u00D7")),
present.length > 8 && (React.createElement("input", { autoFocus: true, placeholder: "Filtra voci\u2026", value: optSearch, onChange: e => setOptSearch(e.target.value), onMouseDown: e => e.stopPropagation(), style: { width: "100%", boxSizing: "border-box", background: "#0F0F14", border: "1px solid #1e2a3a", borderRadius: 5, padding: "5px 8px", color: "#e2e8f0", fontSize: 12, outline: "none", marginBottom: 6 } })),
React.createElement("div", { style: { display: "flex", gap: 6, padding: "0 6px 8px", borderBottom: "1px solid #1e2a3a", marginBottom: 4 } },
...
return shown.length === 0 ? React.createElement("div", { style: { padding: "6px 8px", fontSize: 11, color: "#475569" } }, "Nessuna voce") :
shown.sort().map(o => (React.createElement("label", { key: o, style: { display: "flex", alignItems: "center", gap: 8, padding: "5px 8px", borderRadius: 5, cursor: "pointer", fontSize: 12, color: "#e2e8f0" }, onMouseDown: e => e.preventDefault(), onClick: () => toggleVal(c.key, o) },
React.createElement("span", { style: { width: 15, height: 15, flexShrink: 0, borderRadius: 3, border: "1px solid " + (sel.indexOf(o) >= 0 ? "#2DD4BF" : "#475569"), background: sel.indexOf(o) >= 0 ? "#2DD4BF" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: "#0a0a0f", fontWeight: 900 } }, sel.indexOf(o) >= 0 ? "✓" : ""),
React.createElement("span", { style: { flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, c.render ? c.render(o, {}) : o),
React.createElement("span", { style: { fontSize: 10, color: "#475569", fontFamily: "monospace" } }, counts[o]))));
})())))));
})() : (React.createElement("input", { placeholder: "\u2026", value: txt[c.key] || "", onChange: e => setTextF(c.key, e.target.value), style: { background: "#16161C", border: "1px solid #1e2a3a", borderRadius: 4, padding: "3px 6px", color: "#e2e8f0", fontSize: 11, width: "100%", outline: "none" } }))))),
...
const BADGE = {
non_abilitato: { t: "Non abilitato", c: "#94a3b8", bg: "#94a3b815" },
...
errore: { t: "Stato non disponibile", c: "#94a3b8", bg: "#94a3b815" },
};
const b = BADGE[stato] || { t: "Verifico…", c: "#64748b", bg: "#64748b15" };
return (React.createElement("div", { style: { border: "1px solid #1e2a3a", borderRadius: 10, padding: "13px 15px", background: "#0D0D12" } },
React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" } },
React.createElement("div", { style: { fontSize: 12.5, fontWeight: 800, color: "#e2e8f0" } }, "\uD83D\uDD11 Portale cliente"),
...
stato === "attivo" && React.createElement(Btn, { sm: true, variant: "ghost", disabled: busy, onClick: revoca }, busy ? "…" : "Revoca accesso"))) : (React.createElement("div", { style: { fontSize: 11, color: "#64748b", marginTop: 9 } }, "Solo il superuser pu\u00F2 abilitare o revocare l'accesso al portale.")),
...
isSuperuser && stato === "non_abilitato" && (React.createElement("div", { style: { fontSize: 11, color: "#64748b", marginTop: 8, lineHeight: 1.5 } }, "User\u00E0 l'email dell'anagrafica qui sopra. Il cliente entrer\u00E0 nel portale con quella email (link di accesso): al primo ingresso viene collegato automaticamente."))));
}
...
return { nAssets: myAssets.length, confPct, conformi, nonConformi, maiVerificati, scaduti, d30, d60, d90, mttr, nChiusi: chiusi.length, ritorniPct, months, top, topCust };
}, [assets, jobs, customers, iecReports, funcReports, parts, custFilter, period]);
const card = { background: "#16161C", border: "1px solid #1e293b", borderRadius: 14, padding: "14px 16px" };
const big = { fontSize: 26, fontWeight: 900, lineHeight: 1.1 };
const sub = { fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: ".06em", marginTop: 4 };
const selS = { background: "#0F141A", color: "#e2e8f0", border: "1px solid #1e293b", borderRadius: 8, padding: "8px 10px", fontSize: 13 };
...
React.createElement("div", { style: Object.assign(Object.assign({}, big), { color: K.confPct === null ? "#64748b" : K.confPct >= 90 ? "#22c55e" : K.confPct >= 70 ? "#f59e0b" : "#ef4444" }) }, K.confPct === null ? "—" : K.confPct + "%"),
React.createElement("div", { style: sub }, "Parco conforme"),
...
React.createElement("div", { style: sub }, "Verifiche scadute"),
React.createElement("div", { style: { fontSize: 11, color: "#64748b", marginTop: 6 } },
K.d30,
" entro 30g \u00B7 ",
K.d60,
...
React.createElement("div", { style: sub }, "Tempo medio riparazione"),
React.createElement("div", { style: { fontSize: 11, color: "#64748b", marginTop: 6 } },
K.nChiusi,
" correttivi chiusi nel periodo")),
React.createElement("div", { style: card },
React.createElement("div", { style: Object.assign(Object.assign({}, big), { color: K.ritorniPct === null ? "#e2e8f0" : K.ritorniPct <= 10 ? "#22c55e" : K.ritorniPct <= 25 ? "#f59e0b" : "#ef4444" }) }, K.ritorniPct === null ? "—" : K.ritorniPct + "%"),
React.createElement("div", { style: sub }, "Ritorni entro 30 giorni"),
React.createElement("div", { style: { fontSize: 11, color: "#64748b", marginTop: 6 } }, "stesso apparecchio, nuovo guasto"))),
...
React.createElement("div", { style: { fontSize: 9, color: "#64748b", textAlign: "center", marginTop: 3 } },
m.label,
...
K.top.length === 0 ? React.createElement("div", { style: { fontSize: 12, color: "#64748b" } }, "Nessun correttivo nel periodo.") :
K.top.map(t => (React.createElement("div", { key: t.id, style: { display: "flex", justifyContent: "space-between", gap: 10, padding: "6px 0", borderBottom: "1px solid #1e293b", fontSize: 13 } },
React.createElement("div", { style: { minWidth: 0 } },
React.createElement("div", { style: { fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } },
t.name,
" ",
t.code ? React.createElement("span", { style: { fontFamily: "monospace", fontSize: 11, color: "#2DD4BF" } }, t.code) : null),
React.createElement("div", { style: { fontSize: 11, color: "#64748b" } }, t.cust)),
...
K.topCust.length === 0 ? React.createElement("div", { style: { fontSize: 12, color: "#64748b" } }, "Nessun intervento nel periodo.") :
K.topCust.map((c, i) => (React.createElement("div", { key: i, style: { display: "flex", justifyContent: "space-between", gap: 10, padding: "6px 0", borderBottom: "1px solid #1e293b", fontSize: 13 } },
React.createElement("div", { style: { fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, c.name),
React.createElement("div", { style: { fontSize: 12, color: "#94a3b8", whiteSpace: "nowrap" } },
...
const nuovo = withCreateMeta(Object.assign(Object.assign({ status: "operativo", serviceInterval: 6, intervalIec: 12, intervalFunc: 12 }, rec), { customerId }));
if (!nuovo.assetCode) {
...
const selStyle = { width: "100%", background: "#0F141A", color: "#e2e8f0", border: "1px solid #1e293b", borderRadius: 8, padding: "8px 10px", fontSize: 13 };
const lab = { fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 4 };
if (step === 3 && result)
return (React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 14 } },
React.createElement("div", { style: { fontSize: 15, fontWeight: 600, color: "#22c55e" } }, "\u2713 Importazione completata"),
React.createElement("div", { style: { fontSize: 14, color: "#cbd5e1", lineHeight: 1.7 } },
"Creati: ",
...
React.createElement("div", { style: { fontSize: 12, color: "#64748b" } }, "Ricorda di Sincronizzare per portare tutto sul cloud."),
...
React.createElement("div", { style: { fontSize: 13, color: mapping[f.k] >= 0 ? "#e2e8f0" : "#475569" } }, f.label),
...
React.createElement("div", { style: { fontSize: 14, color: "#cbd5e1", lineHeight: 1.6 } },
...
React.createElement("label", { style: { display: "block", border: "2px dashed #1e293b", borderRadius: 12, padding: "28px 16px", textAlign: "center", cursor: "pointer", color: "#94a3b8", fontSize: 14 } },
...
"(",
React.createElement("a", { href: "#", onClick: e => { e.preventDefault(); scaricaTemplateCSV(); }, style: { color: "#94a3b8" } }, "versione CSV per computer"),
")"))));
}
...
f.items.length > 0 && (React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 70px 90px 60px auto", gap: 6, padding: "0 10px 4px", fontSize: 9.5, color: "#64748b", textTransform: "uppercase", letterSpacing: .5, fontWeight: 700 } },
...
photos.length === 0 ? (React.createElement("div", { style: { textAlign: "center", color: "#475569", padding: "16px 0", fontSize: 12 } }, "Nessuna foto. Le foto vengono compresse e, quando sei online, caricate sul cloud: non occupano spazio sul dispositivo.")) : (React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 8 } }, photos.map(photo => (React.createElement("div", { key: photo.id, style: { position: "relative", aspectRatio: "1", borderRadius: 8, overflow: "hidden", border: "1px solid #2a3040" } },
...
React.createElement("span", { style: { fontFamily: "monospace", fontSize: 11, color: "#64748b" } }, jobShortCode(j)),
...
return (React.createElement("div", { style: { marginTop: 12, paddingTop: 12, borderTop: "1px solid #1e2a3a" } },
React.createElement(Btn, { sm: true, disabled: busy, onClick: migra }, busy ? "Sposto…" : ("☁ Sposta " + locali + (locali === 1 ? " foto" : " foto") + " sul cloud")),
React.createElement("div", { style: { fontSize: 11, color: "#64748b", marginTop: 7, lineHeight: 1.5 } }, "Carica le foto dei job sul cloud e qui tiene solo il link: liberi quasi tutto lo spazio e le foto si vedono da ogni dispositivo."),
msg && React.createElement("div", { style: { fontSize: 11.5, color: msg.indexOf("✗") !== -1 ? "#f59e0b" : "#2DD4BF", marginTop: 8 } }, msg)));
}
function SettingsModal({ data, onReplaceJobs, company, onUpdateCompany, onImport, onMerge, onReset, onClose, onCloudPull, isAdmin, isSuperuser, onOpenCestino }) {
...
(() => {
const archJobs = (data.jobs || []).filter(j => j.type === "verifica" || j.type === "calibrazione");
return (React.createElement("div", { style: { background: "#16161C", border: "1px solid #1e293b", borderRadius: 14, padding: "14px 16px", marginTop: 12 } },
React.createElement("div", { style: { fontSize: 13, fontWeight: 700, marginBottom: 6 } }, "\uD83E\uDDF9 Pulizia job di archivio"),
React.createElement("div", { style: { fontSize: 12, color: "#94a3b8", lineHeight: 1.5, marginBottom: 10 } },
"In passato ogni verbale salvato creava anche un job di archivio (tipo \"verifica\"). Ora non pi\u00F9: il job nasce solo quando una verifica eseguita risulta ",
React.createElement("b", null, "NON CONFORME"),
" (correttivo, aperto). Qui sposti nel cestino i vecchi job di archivio \u2014 al momento ne hai ",
React.createElement("b", { style: { color: "#5EEAD4" } }, archJobs.length),
"."),
React.createElement(Btn, { sm: true, onClick: () => {
if (archJobs.length === 0) {
alert("Nessun job di archivio da pulire.");
return;
}
if (!window.confirm("Spostare nel cestino " + archJobs.length + " job di tipo verifica/calibrazione? Restano nel cestino, recuperabili."))
return;
const ids = {};
archJobs.forEach(j => { ids[j.id] = true; });
if (onReplaceJobs)
onReplaceJobs((data.jobs || []).filter(j => !ids[j.id]));
}, disabled: archJobs.length === 0 },
"Sposta nel cestino (",
archJobs.length,
")")));
})(),
...
React.createElement("span", { style: { fontSize: 12.5, color: "#e2e8f0", fontWeight: 700 } },
fmtMB(u.chars),
" usati"),
React.createElement("span", { style: { fontSize: 12.5, fontWeight: 800, color: col, fontFamily: "monospace" } },
u.pct,
"%")),
React.createElement("div", { style: { height: 10, background: "#0D0D12", border: "1px solid #1e2a3a", borderRadius: 6, overflow: "hidden" } },
React.createElement("div", { style: { width: pc + "%", height: "100%", background: col, transition: "width .3s" } })),
React.createElement("div", { style: { fontSize: 11.5, color: "#94a3b8", marginTop: 9, lineHeight: 1.55 } },
"Spazio del browser su questo dispositivo (limite tipico ~",
fmtMB(u.budget),
", stima prudente). Di cui foto: ",
React.createElement("b", { style: { color: "#e2e8f0" } }, fmtMB(ph.chars)),
ph.count > 0 ? " (" + ph.count + " foto)" : "",
"."),
React.createElement("div", { style: { fontSize: 11, color: "#64748b", marginTop: 6, lineHeight: 1.5 } }, "Le foto pesano molto pi\u00F9 dei dati. Sopra l'80% l'app ti avvisa; se lo spazio si riempie, i salvataggi falliscono. In pi\u00F9 una copia di sicurezza viene scritta da sola nella memoria del dispositivo (IndexedDB): se il browser svuotasse i dati del sito, al riavvio si ripristinano automaticamente. Tieni comunque un backup recente (qui sotto).")));
})(),
...
React.createElement("div", { style: { width: '100%', maxWidth: 430, background: '#141418', border: '1px solid #2A2A38', borderRadius: 14, padding: 32, textAlign: 'center' } },
React.createElement("div", { style: { fontSize: 40, marginBottom: 10 } }, "\uD83C\uDF89"),
React.createElement("h2", { style: { fontSize: 19, fontWeight: 800, color: '#F0F0F5', marginBottom: 8 } }, "Organizzazione creata!"),
React.createElement("p", { style: { fontSize: 13, color: '#94a3b8', lineHeight: 1.55, marginBottom: 18 } },
"Sei l'amministratore (superuser) di ",
React.createElement("strong", { style: { color: '#e2e8f0' } }, orgName.trim()),
". Questo \u00E8 il ",
...
catch (e) { } }, style: { background: '#0D0D12', border: '1px dashed #2DD4BF66', borderRadius: 10, padding: '14px 10px', fontFamily: 'monospace', fontSize: 20, fontWeight: 800, letterSpacing: 2, color: '#2DD4BF', cursor: 'pointer', marginBottom: 8, wordBreak: 'break-all' } }, createdCode),
React.createElement("div", { style: { fontSize: 11, color: copied ? '#2DD4BF' : '#5A5A70', marginBottom: 22 } }, copied ? '✓ Copiato negli appunti' : 'Tocca per copiare · Conservalo: serve ai colleghi per registrarsi'),
React.createElement("button", { onClick: () => { if (createdUser)
onLogin(createdUser);
else
window.location.reload(); }, style: { width: '100%', background: 'linear-gradient(135deg,#2DD4BF,#0D9488)', color: '#000', border: 'none', borderRadius: 8, padding: 13, fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' } }, "Entra in MedTrace \u2192"))));
}
return (React.createElement("div", { style: { minHeight: '100vh', background: '#0D0D12', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 } },
...
React.createElement("h2", { style: { fontSize: 18, fontWeight: 800, marginBottom: 24, color: '#F0F0F5' } }, mode === 'login' ? 'Accedi' : mode === 'create' ? 'Crea la tua organizzazione' : 'Crea account'),
...
React.createElement("div", { style: { fontSize: 11, color: '#5A5A70', marginTop: 6, lineHeight: 1.4 } }, "Ti \u00E8 stato consegnato all'acquisto di MedTrace. Creando l'organizzazione ne diventi l'amministratore (superuser); il codice invito per i colleghi te lo mostro subito dopo.")))),
...
React.createElement("button", { onClick: handle, disabled: loading, style: { width: '100%', background: 'linear-gradient(135deg,#2DD4BF,#0D9488)', color: '#000', border: 'none', borderRadius: 8, padding: 13, fontSize: 14, fontWeight: 800, cursor: loading ? 'wait' : 'pointer', opacity: loading ? .7 : 1, fontFamily: 'inherit' } }, loading ? 'Caricamento…' : mode === 'login' ? 'Accedi' : mode === 'create' ? 'Crea organizzazione' : 'Crea account'),
...
return (React.createElement("div", { key: j.id, style: { background: '#141418', border: `1px solid ${j.status === 'chiuso' ? '#22c55e22' : j.status === 'aperto' ? '#ef444422' : (j.status || '').indexOf('attesa') >= 0 ? '#f59e0b33' : '#2A2A38'}`, borderRadius: 10, padding: '14px 16px' } },
...
const [tapeW, setTapeW] = React.useState(24);
...
const fCode = tapeW >= 24 ? 13 : (tapeW >= 18 ? 10 : 7);
...
let info = "";
if (isAssetSticker) {
info = '<div class="codeBig">' + code + '</div>';
}
else {
info = tc.showName
...
React.createElement("span", { style: { fontSize: 12.5, color: "#94a3b8", fontWeight: 600 } }, "Larghezza nastro:"),
[24, 18, 12].map(wmm => (React.createElement("button", { key: wmm, type: "button", onClick: () => setTapeW(wmm), style: { background: tapeW === wmm ? "#2DD4BF22" : "#141418", border: "1px solid " + (tapeW === wmm ? "#2DD4BF" : "#2a3040"), color: tapeW === wmm ? "#2DD4BF" : "#94a3b8", borderRadius: 7, padding: "7px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", touchAction: "manipulation" } },
wmm,
" mm")))),
React.createElement("div", { style: { fontSize: 11, color: "#64748b", lineHeight: 1.5, marginBottom: 14 } },
"Stampa \u2192 nella finestra scegli la tua stampante per etichette (nastri fino a ",
React.createElement("b", null, "24 mm"),
"; su Android pu\u00F2 servire il plugin di stampa del produttore). Fai una prova: se l'etichetta non \u00E8 centrata, si calibra."),
React.createElement("div", { style: FORM_FOOTER },
React.createElement("button", { onClick: onClose, style: FORM_BTN_GHOST }, "Chiudi"),
...
netOff && (React.createElement("div", { style: { marginBottom: 12, padding: "10px 13px", background: "#1c1607", border: "1px solid #f59e0b44", borderRadius: 9, color: "#fbbf24", fontSize: 12, lineHeight: 1.5 } }, "\uD83D\uDCF4 Sei offline: le richieste vivono sul cloud, quindi l'elenco potrebbe non essere aggiornato e le azioni richiedono connessione.")),
...
r.contact),
React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } },
!done && React.createElement("button", { disabled: busy, onClick: () => doConvert(r), style: { background: busy ? "#2a3040" : "#2DD4BF", color: busy ? "#64748b" : "#0a0a0f", border: "none", borderRadius: 8, padding: "9px 14px", fontSize: 13, fontWeight: 800, cursor: busy ? "default" : "pointer", touchAction: "manipulation" } }, busy ? "…" : "✓ Accetta e crea job"),
...
const SECTION_ALIAS = { scadenze: "agenda", help: "procedures" };
...
const cleanupAutoJobs = () => {
if (checkLocked())
return;
const match = jobs.filter(j => j.type === "verifica" || j.type === "calibrazione");
if (match.length === 0) {
showToast("Nessun job di archivio da pulire");
return;
}
appConfirm("Spostare nel cestino " + match.length + " job di tipo verifica/calibrazione? Restano recuperabili dal cestino per 90 giorni.", () => {
const ids = {};
match.forEach(j => { ids[j.id] = true; moveToTrash("jobs", j); });
setJobs(js => js.filter(j => !ids[j.id]));
showToast(match.length + " job spostati nel cestino", "#f59e0b");
});
};
...
return _su.pct >= 85 ? (React.createElement("div", { style: { marginBottom: 28, padding: "12px 14px", background: "#160d0d", border: "1px solid #7f1d1d", borderLeft: "3px solid #ef4444", borderRadius: 8, display: "flex", justifyContent: "space-between", alignItems: "center" } },
React.createElement("div", null,
React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: "#fecaca" } },
"\uD83D\uDCBE Spazio dati al ",
_su.pct,
"%"),
React.createElement("div", { style: { fontSize: 11, color: "#94a3b8", marginTop: 2 } }, _su.pct >= 100 ? "I salvataggi stanno fallendo: esporta un backup ed elimina foto pesanti dai job." : "Quasi pieno: esporta un backup. Dettagli in Impostazioni → Spazio dati locale.")),
...
React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 6 } },
React.createElement("strong", { style: { color: "#e2e8f0", fontSize: 14, flex: 1, minWidth: 0, wordBreak: "break-word" } }, a.name || jobTipoLabel(j)),
...
" ",
r.technician)),
React.createElement("div", { onClick: (e) => { e.stopPropagation(); r.published ? appConfirm("Ritirare il verbale dal portale? Il cliente non lo vedrà più finché non lo ripubblichi.", () => setReportPublished("func", r.id, false), "danger") : setReportPublished("func", r.id, true); }, style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "8px 14px", borderBottom: "1px solid #1e2a3a", cursor: "pointer", touchAction: "manipulation", WebkitTapHighlightColor: "transparent", background: r.published ? "#22c55e0a" : "#f59e0b0d" } },
...
React.createElement("span", { style: { fontSize: 11, fontWeight: 800, color: r.published ? "#64748b" : "#2DD4BF" } }, r.published ? "Ritira" : "📤 Pubblica")),
...
" ",
r.technician)),
React.createElement("div", { onClick: (e) => { e.stopPropagation(); r.published ? appConfirm("Ritirare il verbale dal portale? Il cliente non lo vedrà più finché non lo ripubblichi.", () => setReportPublished("iec", r.id, false), "danger") : setReportPublished("iec", r.id, true); }, style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "8px 14px", borderBottom: "1px solid #1e2a3a", cursor: "pointer", touchAction: "manipulation", WebkitTapHighlightColor: "transparent", background: r.published ? "#22c55e0a" : "#f59e0b0d" } },
React.createElement("span", { style: { fontSize: 11, fontWeight: 700, color: r.published ? "#22c55e" : "#f59e0b" } }, r.published ? "👁 Pubblicato al cliente" : "🔒 Bozza — non visibile al cliente"),
React.createElement("span", { style: { fontSize: 11, fontWeight: 800, color: r.published ? "#64748b" : "#2DD4BF" } }, r.published ? "Ritira" : "📤 Pubblica")),
React.createElement("div", { style: { display: "grid", gridTemplateColumns: "40px 1fr 1fr 36px", gap: 0, background: "#0D0D12" } },
...
if (info.scaduto) {
return (React.createElement("div", { style: { position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 9998, background: "#1a0d0d", borderTop: "1px solid #ef444466", color: "#fca5a5", fontSize: 12, fontWeight: 700, textAlign: "center", padding: "7px 12px" } }, "\u26D4 Abbonamento scaduto \u2014 l'app \u00E8 in sola lettura. I tuoi dati restano al sicuro e consultabili: rinnova per riattivare le modifiche."));
}
if (typeof g === "number" && g <= 30) {
return (React.createElement("div", { style: { position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 9998, background: "#1c1607", borderTop: "1px solid #f59e0b55", color: "#fbbf24", fontSize: 12, fontWeight: 700, textAlign: "center", padding: "7px 12px" } },
...
return (React.createElement("div", { style: { background: "#141418", border: "1px solid #1e2a3a", borderRadius: 12, padding: "14px 16px", marginBottom: 14 } },
React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" } },
React.createElement("span", { style: { fontSize: 12.5, fontWeight: 800, color: "#e2e8f0" } },
"\uD83D\uDCC5 Licenza",
...
if (isOff || !giu)
return null;
return (React.createElement("div", { style: { position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 9999, background: "#1c1607", borderTop: "1px solid #f59e0b55", color: "#fbbf24", fontSize: 12, fontWeight: 700, textAlign: "center", padding: "7px 12px" } }, "\uD83D\uDCF4 Sei offline \u2014 stai lavorando sui dati locali. Quando torna la rete, ricordati di Sincronizzare."));
}
function App() {
