...
let cestinati = fused.filter(r => r.deleted);
const LIMITE_LAPIDI = Date.now() - 90 * 24 * 60 * 60 * 1000;
const daPurgare = cestinati.filter(r => { const t = Date.parse(r.deletedAt || r.updatedAt || ""); return t && t < LIMITE_LAPIDI; });
if (daPurgare.length > 0) {
const ids = {};
daPurgare.forEach(r => { ids[r.id] = true; });
cestinati = cestinati.filter(r => !ids[r.id]);
try {
yield client.from(table).delete().in("id", daPurgare.map(r => r.id));
}
catch (e) { }
}
merged[stateKey] = attivi;
mergedTrash[stateKey] = cestinati;
const rows = [...attivi, ...cestinati].map(r => { const s = toSnakeRecord(r); s.org_id = orgId; return s; });
if (rows.length > 0) {
...
const [selectedAssetId, setSelectedAssetId] = useState(propAssetId || (initial === null || initial === void 0 ? void 0 : initial.assetId) || "");
...
const [templateId, setTemplateId] = useState((initial === null || initial === void 0 ? void 0 : initial.templateId) || suggestedTpl || "generico");
...
const getMeasures = useCallback((norm, cls, patientType, method, sfc) => {
if (norm === "61010")
return [
...
useEffect(() => { setF(x => (Object.assign(Object.assign({}, x), { measures: getMeasures(x.norm, x.equipClass, x.patientType, x.leakageMethod, x.sfc !== false) }))); }, [f.norm, f.equipClass, f.patientType, f.leakageMethod, f.sfc]);
const computePass = useCallback((measures, visual) => {
...
useEffect(() => { setF(x => (Object.assign(Object.assign({}, x), { overallPass: computePass(x.measures, x.visual) }))); }, [f.measures, f.visual]);
...
const [sort, setSort] = useState({ key: defaultSort || ((_a = cols[0]) === null || _a === void 0 ? void 0 : _a.key), dir: "asc" });
...
const [f, setF] = useState(initial ? Object.assign(Object.assign({}, blank), initial) : blank);
const [errors, setErrors] = useState({});
...
const [errors, setErrors] = useState({});
const [showAdv, setShowAdv] = useState(!!initial);
const s = k => e => setF(x => (Object.assign(Object.assign({}, x), { [k]: e.target.value })));
...
const [f, setF] = useState(initial ? Object.assign(Object.assign(Object.assign({}, blank), initial), { sellPrice: initial.sellPrice || initial.unitPrice, markupPct: initial.markupPct || 0, compatible: initial.compatible || [] }) : blank);
const s = k => e => setF(x => (Object.assign(Object.assign({}, x), { [k]: e.target.value })));
...
const [f, setF] = useState(initial ? Object.assign(Object.assign(Object.assign({}, blank), initial), { timeline: initial.timeline || [], photos: initial.photos || [], parts: initial.parts || [] }) : blank);
const [errors, setErrors] = useState({});
const s = k => e => setF(x => (Object.assign(Object.assign({}, x), { [k]: e.target.value })));
useEffect(() => {
...
const [f, setF] = useState(initial ? Object.assign(Object.assign(Object.assign({}, blank), initial), { items: initial.items || [], jobIds: initial.jobIds || [] }) : blank);
...
const [steps, setSteps] = useState(job.timeline || []);
const [photos, setPhotos] = useState(job.photos || []);
const [newStep, setNewStep] = useState({ date: new Date().toISOString().slice(0, 10), type: "intervento", note: "", hours: 0 });
...
var _a, _b;
const [items, setItems] = useState([{ partId: preselectPartId || ((_a = parts[0]) === null || _a === void 0 ? void 0 : _a.id) || "", qty: 1 }]);
const [errors, setErrors] = useState({});
const [reason, setReason] = useState("");
const [assetId, setAssetId] = useState(((_b = assets[0]) === null || _b === void 0 ? void 0 : _b.id) || "");
...
const [activeRole, setActiveRole] = useState(((_a = editableRoles[0]) === null || _a === void 0 ? void 0 : _a.id) || "admin");
...
const [form, setForm] = useState(Object.assign(Object.assign({}, instrument), { calDate: today, calExpiry: (() => { const d = new Date(today); d.setMonth(d.getMonth() + (instrument.calInterval || 12)); return d.toISOString().slice(0, 10); })(), certNumber: '', calLab: instrument.calLab || '' }));
const set = (k, v) => setForm(f => (Object.assign(Object.assign({}, f), { [k]: v })));
const INP = FORM_INP;
const LBL = FORM_LBL;
return (React.createElement("div", null,
...
"Stampa \u2192 nella finestra scegli la stampante ",
React.createElement("b", null, "Brother"),
" (su Android serve il ",
React.createElement("b", null, "Brother Print Service Plugin"),
"). Fai una prova: se l'etichetta non \u00E8 centrata, si calibra."),
React.createElement("div", { style: FORM_FOOTER },
React.createElement("button", { onClick: onClose, style: FORM_BTN_GHOST }, "Chiudi"),
...
useEffect(() => { const on = () => setNetOff(false), off = () => setNetOff(true); window.addEventListener("online", on); window.addEventListener("offline", off); return () => { window.removeEventListener("online", on); window.removeEventListener("offline", off); }; }, []);
...
useEffect(() => { _instrumentsRegistry = instruments || []; }, [instruments]);
const [procedures, setProcedures] = useState(initialData.procedures || []);
...
const [withdrawals, setWDs] = useState(initialData.withdrawals);
...
const [iecReports, setIecReports] = useState(initialData.iecReports || []);
const [funcReports, setFuncReports] = useState(initialData.funcReports || []);
...
const [cestino, setCestino] = useState(initialData.cestino || CESTINO_VUOTO);
useEffect(() => {
...
const [mobileSearch, setMobileSearch] = useState(() => loadFilterState("search", { assets: "", jobs: "", customers: "", parts: "", invoices: "", iec: "", func: "" }));
const [jobFilter, setJobFilter] = useState(() => loadFilterState("jobFilter", "open"));
...
const [assetMobileView, setAssetMobileView] = useState(() => {
try {
...
const MONTHS_IT = ["Gennaio", "Febbraio", "Marzo", "Aprile", "Maggio", "Giugno", "Luglio", "Agosto", "Settembre", "Ottobre", "Novembre", "Dicembre"];
const scheduleMonths = useMemo(() => MONTHS_IT.map((monthLabel, month) => ({
month, monthLabel,
...
const scadenzeVerifiche = useMemo(() => {
const today = new Date();
today.setHours(0, 0, 0, 0);
const out = [];
...
useEffect(() => { loadRichieste(); }, []);
useEffect(() => { if (tab === "richieste")
loadRichieste(); }, [tab]);
...
const filteredAssets = useMemo(() => assets.filter(a => !search || [a.name, a.brand, a.model, a.serial, a.location].join(" ").toLowerCase().includes(search.toLowerCase())), [assets, search]);
const filteredParts = useMemo(() => parts.filter(p => !search || [p.code, p.name, p.brand, p.location].join(" ").toLowerCase().includes(search.toLowerCase())), [parts, search]);
const filteredOrders = useMemo(() => orders.filter(o => !search || [o.id, o.supplier, o.notes].join(" ").toLowerCase().includes(search.toLowerCase())), [orders, search]);
const filteredCustomers = useMemo(() => customers.filter(c => !search || [c.name, c.vat, c.email, c.contact].join(" ").toLowerCase().includes(search.toLowerCase())), [customers, search]);
