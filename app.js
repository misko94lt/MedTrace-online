...
admin: ["dashboard", "jobs", "iec", "func", "agenda", "scadenze", "assets", "instruments", "customers", "parts", "invoices", "procedures", "help"],
finance: ["dashboard", "jobs", "assets", "customers", "invoices", "procedures", "help"],
tecnico: ["dashboard", "jobs", "iec", "func", "agenda", "scadenze", "assets", "instruments", "customers", "parts", "procedures", "help"],
};
...
function AssetDetailModal({ asset, jobs, parts, iecReports, funcReports, customers, onClose, onEditAsset, onNewJob, onNewIec, onNewFunc, onAssetSticker, onOpenJob, company, generateJobPDF, generateIECPDF, generateFuncPDF, templates }) {
...
alert("DEBUG 1: onChange partito. File: " + (file ? file.name + " (" + file.size + " byte)" : "NESSUNO"));
if (!file) {
input.value = '';
return;
}
const reader = new FileReader();
reader.onload = ev => {
alert("DEBUG 2: file letto, lunghezza testo: " + (ev.target.result ? ev.target.result.length : 0));
try {
const imported = JSON.parse(ev.target.result);
alert("DEBUG 3: JSON ok. Apparecchi nel file: " + ((imported.assets || []).length));
...
onImport(imported);
alert("DEBUG 4: onImport chiamato.");
}
catch (err) {
alert('DEBUG ERRORE parse: ' + ((err === null || err === void 0 ? void 0 : err.message) || 'JSON non valido'));
}
input.value = '';
};
reader.onerror = () => { alert('DEBUG: errore lettura file'); input.value = ''; };
reader.readAsText(file);
};
...
React.createElement("div", { style: { marginTop: 14, padding: "12px 14px", background: "#0D0D12", border: "1px solid #1e2a3a", borderRadius: 9 } },
React.createElement("div", { style: { fontSize: 12, color: "#e2e8f0", fontWeight: 700, marginBottom: 4 } }, "Migra un backup nel cloud"),
React.createElement("div", { style: { fontSize: 11, color: "#94a3b8", lineHeight: 1.5, marginBottom: 10 } }, "Hai un backup esportato dalla versione offline? Caricalo qui: i dati vengono inviati direttamente al cloud, in un colpo solo."),
React.createElement("input", { ref: migRef, type: "file", accept: "application/json,.json", onChange: fileRefMigrate, style: { display: "none" } }),
React.createElement(Btn, { sm: true, disabled: cloudBusy, onClick: () => migRef.current && migRef.current.click() }, cloudBusy ? "Migrazione…" : "⬆ Importa file e carica su cloud")),
...
React.createElement("label", null,
React.createElement("input", { type: "file", accept: ".json,application/json,text/plain,*/*", onChange: fileRef, style: { display: "none" } }),
...
React.createElement("input", { type: "file", accept: ".json,application/json,text/plain,*/*", onChange: fileRefMerge, style: { display: "none" } }),
...
<div class="row">ID: <b>${(asset === null || asset === void 0 ? void 0 : asset.id) || "—"}</b></div>
...
React.createElement(AssetDetailModal, { asset: modal.data, jobs: jobs, parts: parts, iecReports: iecReports, funcReports: funcReports, customers: customers, company: company, templates: allTemplates, generateIECPDF: generateIECPDF, generateFuncPDF: generateFuncPDF, onClose: popModal, onEditAsset: () => pushModal({ type: "asset", data: modal.data }), onNewJob: () => pushModal({ type: "job", data: { assetId: modal.data.id, type: "correttiva", priority: "normale", status: "aperto", description: "", openDate: new Date().toISOString().slice(0, 10), parts: [], laborHours: 0, laborRate: 55, notes: "", timeline: [], photos: [] } }), onNewIec: () => pushModal({ type: "iec", assetId: modal.data.id, data: null }), onNewFunc: () => pushModal({ type: "func", assetId: modal.data.id, data: null }), onAssetSticker: () => pushModal({ type: "sticker", data: {}, kind: "asset", assetId: modal.data.id }), onOpenJob: j => pushModal({ type: "job", data: j }) }))),
