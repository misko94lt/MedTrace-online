...
setFileErr("Per leggere i file Excel (.xlsx) serve una riga in più in index.html (te la do io). In alternativa: in Excel fai \"Salva con nome → CSV\" e carica quello.");
return;
}
const fr = new FileReader();
fr.onload = () => {
...
const hs = IMP_FIELDS.map(f => f.label);
const esempio = ["Defibrillatore", "Philips", "HeartStart XL", "US00123456", "", "Piano 1 — Ambulatorio 3", "operativo", "15/01/2026", "15/07/2026", "6", "12", "12", "IEC 62353", "I", "BF", "C", "diretto", "10/03/2022", "01/02/2022", "20/03/2022", "10/03/2027", "8500", "MedSupply Srl", "", "", "", "", "Acquistato nuovo"];
if (typeof window.XLSX !== "undefined") {
const ws = window.XLSX.utils.aoa_to_sheet([hs, esempio]);
...
const csv = "\ufeff" + [hs, esempio].map(r => r.map(c => '"' + String(c).replace(/"/g, '""') + '"').join(";")).join("\r\n");
...
"Parti da zero? ",
React.createElement("a", { href: "#", onClick: e => { e.preventDefault(); scaricaTemplate(); }, style: { color: "#2DD4BF" } }, "Scarica il modello"),
" con tutte le 27 colonne e una riga d'esempio \u2014 compili solo quelle che ti servono (obbligatorio solo il Nome).")));
}
