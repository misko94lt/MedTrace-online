...
{ id: "invoices", label: "Preventivi / Fatture" },
{ id: "procedures", label: "Procedure" },
{ id: "help", label: "Guida / Help" },
];
const DEFAULT_ROLE_PERMS = {
superuser: PERM_SECTIONS.map(s => s.id),
admin: ["dashboard", "jobs", "iec", "func", "agenda", "assets", "instruments", "customers", "parts", "invoices", "procedures", "help"],
finance: ["dashboard", "jobs", "assets", "customers", "invoices", "procedures", "help"],
tecnico: ["dashboard", "jobs", "iec", "func", "agenda", "assets", "instruments", "customers", "parts", "procedures", "help"],
};
...
v = null;
if (["labor_hours", "labor_rate", "markup_pct", "min_qty", "qty", "sell_price", "service_interval", "interval_iec", "interval_func", "total", "unit_price", "price", "cost"].includes(sk) && v === "")
v = null;
out[sk] = v;
}
return out;
...
window.dispatchEvent(new CustomEvent("toast", { detail: { msg: "Excel non disponibile offline, uso CSV", color: "#f59e0b" } }));
...
/* ── ESITO ── */
.verdict { display: flex; align-items: center; justify-content: center; gap: 10px; padding: 14px; border-radius: 8px; margin: 16px 0; font-weight: 800; font-size: 16px; letter-spacing: .5px; }
...
/* ── FOOTER ── */
.footer { margin-top: 24px; padding-top: 11px; border-top: 2px solid #0E7490; display: flex; justify-content: space-between; font-size: 8.5px; color: #94A3B8; }
`;
...
.map(r => {
var _a, _b;
return ({ date: r.date || "", kind: "Sicurezza elettrica", norm: r.norm === '61010' ? 'IEC 61010' : 'EN 62353',
...
function FuncVerifyForm({ initial, assetId: propAssetId, assets, customers, existingReports, templates, onSave, onClose }) {
...
"CANCELLAZIONE: puoi azzerare tutti i dati locali da Impostazioni → Zona pericolo. In modalità online la rimozione dal cloud dipende dalla configurazione della tua organizzazione.",
"I testi di Privacy e Termini d'uso completi sono richiamabili dai due pulsanti qui sotto.",
...
const PRIVACY_TEXT = `Ultimo aggiornamento: maggio 2026
Questa informativa spiega in modo semplice come MedTrace tratta i dati. Non costituisce consulenza legale: per un uso professionale è consigliabile farla rivedere da un consulente privacy.
1. CHI TRATTA I DATI
...
• Versione online: oltre al dispositivo, i dati sono salvati su un servizio cloud (Supabase) per consentire login, sincronizzazione tra dispositivi e condivisione tra i membri della stessa organizzazione.
4. FINALITÀ
...
5. CONSERVAZIONE
I dati restano finché li mantieni nell'app. Puoi esportarli, modificarli o cancellarli in qualsiasi momento dalle Impostazioni. Conserva i dati personali solo per il tempo necessario alle tue finalità e agli obblighi di legge.
...
8. CONDIVISIONE
MedTrace non vende né cede i tuoi dati a terzi. Nella versione online il fornitore dell'infrastruttura cloud (Supabase) agisce come responsabile del trattamento per la sola conservazione tecnica.
Per domande su questa informativa, contatta chi gestisce la tua installazione di MedTrace.`;
const TERMS_TEXT = `Ultimo aggiornamento: maggio 2026
...
7. PROPRIETÀ
I dati che inserisci restano tuoi. Il software e il suo codice restano di proprietà di chi lo ha sviluppato; l'uso ti è concesso in licenza per le finalità gestionali previste.
8. MODIFICHE
...
React.createElement(ErrorSummary, { errors: errors }),
React.createElement(Grid, { cols: "1fr 1fr" },
React.createElement(Inp, { label: "Nome / Tipo apparecchio", value: f.name, onChange: s("name"), onBlur: e => autofillFrom("name", e.target.value), list: "dl-names", placeholder: "es. Defibrillatore" }),
React.createElement(Inp, { label: "Marca", value: f.brand, onChange: s("brand"), list: "dl-brands" }),
React.createElement(Inp, { label: "Modello", value: f.model, onChange: s("model"), onBlur: e => autofillFrom("model", e.target.value), list: "dl-models", hint: "Scrivi un modello gi\u00E0 inserito: marca, tipo e norma si compilano da soli." }),
React.createElement("datalist", { id: "dl-names" }, nameOptions.map(o => React.createElement("option", { key: o, value: o }))),
React.createElement("datalist", { id: "dl-brands" }, brandOptions.map(o => React.createElement("option", { key: o, value: o }))),
React.createElement("datalist", { id: "dl-models" }, modelOptions.map(o => React.createElement("option", { key: o, value: o }))),
...
function JobDetailModal({ job, assets, parts, customers, company, onEdit, onTimeline, onClose }) {
const asset = assets.find(a => a.id === job.assetId) || {};
const customer = customers.find(c => c.id === (job.customerId || asset.customerId)) || {};
...
function AssetDetailModal({ asset, jobs, parts, iecReports, funcReports, customers, onClose, onEditAsset, onNewJob, onNewIec, onNewFunc, onOpenJob, company, generateJobPDF, generateIECPDF, generateFuncPDF, templates }) {
...
return (React.createElement("div", { style: { overflowX: "auto", border: "1px solid #1e2a3a", borderRadius: 8 } },
React.createElement("table", { style: { borderCollapse: "collapse", width: "100%", minWidth: 340 } },
React.createElement("thead", null,
React.createElement("tr", { style: { background: "#0D0D12" } },
React.createElement("th", { style: { textAlign: "left", padding: "8px 10px", fontSize: 10, color: "#64748b", textTransform: "uppercase", letterSpacing: .5, fontWeight: 700, position: "sticky", left: 0, background: "#0D0D12" } }, "Sezione"),
editableRoles.map(r => (React.createElement("th", { key: r.id, title: r.desc, style: { padding: "8px 6px", fontSize: 10.5, color: "#cbd5e1", fontWeight: 700, whiteSpace: "nowrap" } }, r.label))))),
React.createElement("tbody", null, PERM_SECTIONS.map((sec, i) => (React.createElement("tr", { key: sec.id, style: { borderTop: "1px solid #16161C", background: i % 2 ? "#101014" : "transparent" } },
React.createElement("td", { style: { padding: "7px 10px", fontSize: 12, color: "#e2e8f0", whiteSpace: "nowrap", position: "sticky", left: 0, background: i % 2 ? "#101014" : "#0a0a0f" } }, sec.label),
editableRoles.map(r => (React.createElement("td", { key: r.id, style: { textAlign: "center", padding: "7px 6px" } },
React.createElement("input", { type: "checkbox", checked: has(r.id, sec.id), onChange: () => toggle(r.id, sec.id), style: { width: 16, height: 16, accentColor: "#2DD4BF", cursor: "pointer" } })))))))))));
}
function SettingsModal({ data, company, onUpdateCompany, onImport, onMerge, onReset, onClose, onCloudPull, isAdmin, isSuperuser }) {
...
setCloudBusy(true);
setCloudStatus("Caricamento dati sul cloud…");
try {
const res = yield supabaseSyncUp(data);
const tot = Object.values(res).reduce((a, b) => a + b, 0);
setCloudStatus("✓ Caricati " + tot + " record sul cloud.");
}
catch (e) {
...
setCloudBusy(true);
setCloudStatus("Scaricamento dati dal cloud…");
try {
...
const Section = ({ icon, title, color, children, accent }) => (React.createElement("div", { style: { background: "#141418", borderRadius: 14, padding: "18px", border: "1px solid " + (accent || "#24242F") } },
...
comp.logo && (React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 9, marginTop: 12, cursor: isAdmin ? "pointer" : "default", fontSize: 12.5, color: "#cbd5e1" } },
React.createElement("input", { type: "checkbox", checked: !!comp.logoHasName, disabled: !isAdmin, onChange: (e) => setComp(x => (Object.assign(Object.assign({}, x), { logoHasName: e.target.checked }))), style: { width: 17, height: 17, accentColor: "#2DD4BF" } }),
...
React.createElement("label", { style: { display: "flex", alignItems: "flex-start", gap: 11, cursor: "pointer" } },
React.createElement("input", { type: "checkbox", checked: !!comp.stickerPrompt, onChange: (e) => { const v = e.target.checked; setComp(x => (Object.assign(Object.assign({}, x), { stickerPrompt: v }))); onUpdateCompany(Object.assign(Object.assign({}, company), { stickerPrompt: v })); }, style: { width: 17, height: 17, accentColor: "#2DD4BF", marginTop: 1, flexShrink: 0 } }),
...
"Spunta quali sezioni pu\u00F2 vedere ogni ruolo. Il ",
React.createElement("strong", { style: { color: "#5EEAD4" } }, "Superuser"),
" vede sempre tutto e pu\u00F2 gestire utenti e dati fiscali. Le modifiche valgono per i nuovi accessi degli utenti."),
...
"I tuoi dati sono salvati su questo dispositivo. Usa i pulsanti per sincronizzarli col cloud:",
React.createElement("strong", { style: { color: "#5EEAD4" } }, " Carica"),
" invia i dati locali al cloud,",
React.createElement("strong", { style: { color: "#5EEAD4" } }, " Scarica"),
" li recupera dal cloud su questo dispositivo."),
...
React.createElement("span", { style: { background: "#1a1a22", color: "#94a3b8", border: "1px solid #2a3040", borderRadius: 9, padding: "9px 16px", cursor: "pointer", fontSize: 13, fontWeight: 700, display: "inline-block" } }, "\u2B06 Importa (sostituisce)")),
React.createElement("label", { title: "Aggiunge i record del file ai tuoi senza cancellare niente" },
React.createElement("input", { type: "file", accept: ".json", onChange: fileRefMerge, style: { display: "none" } }),
React.createElement("span", { style: { background: "#2DD4BF15", color: "#2DD4BF", border: "1px solid #2DD4BF44", borderRadius: 9, padding: "9px 16px", cursor: "pointer", fontSize: 13, fontWeight: 700, display: "inline-block" } }, "\u2295 Unisci da file")))),
...
React.createElement(Section, { icon: "\u26A0\uFE0F", title: "Zona pericolo", color: "#ef4444", accent: "#ef444433" },
...
React.createElement("div", { style: { flex: 1, minWidth: 0, color: "#000", display: "flex", flexDirection: "column", gap: 3 } },
...
React.createElement("div", { style: { fontSize: 11.5, color: "#64748b", lineHeight: 1.5, background: "#1e2a3a44", borderRadius: 8, padding: "10px 12px", marginBottom: 16 } }, supaActive
? React.createElement("span", null,
...
const isOffline = (typeof OFFLINE_MODE !== "undefined" && OFFLINE_MODE);
const isDemo = (typeof DEMO_LOCKED !== "undefined" && DEMO_LOCKED);
return (isOffline || isDemo) ? "superuser" : null;
});
...
if (!annullato && orgCompany && orgCompany.name) {
setCompany(c => (Object.assign(Object.assign({}, c), orgCompany)));
}
}
catch (e) { }
}))();
...
React.createElement("span", { style: { fontSize: 10, color: "#334155", fontFamily: "monospace", letterSpacing: .5 } }, "v0.99"),
...
React.createElement("div", { style: { flex: 1, minWidth: 0 } },
React.createElement("div", { style: { color: "#e2e8f0", fontSize: 13.5, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, displayName),
React.createElement("div", { style: { color: "#64748b", fontSize: 10.5, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, [brandModel, a.serial && ("S/N " + a.serial), a.location].filter(Boolean).join(" · ") || "—")),
...
})())) : (React.createElement(ExcelTable, { exportName: "MedTrace_apparecchi", defaultSort: "name", onRowClick: row => setModal({ type: "assetDetail", data: assets.find(a => a.id === row.id) }), rowBg: row => row.status === "fuori servizio" ? "#ef333308" : row.status === "in manutenzione" ? "#f59e0b08" : "", cols: [
...
(modal === null || modal === void 0 ? void 0 : modal.type) === "jobDetail" && React.createElement(JobDetailModal, { job: modal.data, assets: assets, parts: parts, customers: customers, company: company, onEdit: () => setModal({ type: "job", data: modal.data }), onTimeline: () => setModal({ type: "timeline", data: modal.data }), onClose: popModal }),
...
(modal === null || modal === void 0 ? void 0 : modal.type) === "settings" && React.createElement(SettingsModal, { isAdmin: isAdmin, isSuperuser: isSuperuser, data: { assets, parts, jobs, orders, withdrawals, customers, invoices, quotes, instruments, procedures, iecReports, funcReports, company, _meta: { version: "3.4", exportedAt: new Date().toISOString(), app: "MedTrace" } }, company: company, onUpdateCompany: setCompany, onImport: handleImport, onMerge: handleMerge, onReset: handleReset, onCloudPull: handleCloudPull, onClose: popModal }),
...
(modal === null || modal === void 0 ? void 0 : modal.type) === "sticker" && (React.createElement(Modal, { title: "Sticker QR verifica", onClose: popModal },
React.createElement(StickerModal, { report: modal.data, asset: assets.find(a => { var _a; return a.id === ((_a = modal.data) === null || _a === void 0 ? void 0 : _a.assetId); }), customer: customers.find(c => { var _a; return c.id === ((_a = modal.data) === null || _a === void 0 ? void 0 : _a.customerId); }), company: company, kind: modal.kind, onClose: popModal }))),
...
React.createElement(AssetDetailModal, { asset: modal.data, jobs: jobs, parts: parts, iecReports: iecReports, funcReports: funcReports, customers: customers, company: company, templates: allTemplates, generateIECPDF: generateIECPDF, generateFuncPDF: generateFuncPDF, onClose: popModal, onEditAsset: () => pushModal({ type: "asset", data: modal.data }), onNewJob: () => pushModal({ type: "job", data: { assetId: modal.data.id, type: "correttiva", priority: "normale", status: "aperto", description: "", openDate: new Date().toISOString().slice(0, 10), parts: [], laborHours: 0, laborRate: 55, notes: "", timeline: [], photos: [] } }), onNewIec: () => pushModal({ type: "iec", assetId: modal.data.id, data: null }), onNewFunc: () => pushModal({ type: "func", assetId: modal.data.id, data: null }), onOpenJob: j => pushModal({ type: "job", data: j }) }))),
...
React.createElement(FuncVerifyForm, { initial: modal.data || null, assetId: ((_e = modal.data) === null || _e === void 0 ? void 0 : _e.assetId) || modal.assetId || null, assets: assets, customers: customers, existingReports: funcReports, templates: allTemplates, onSave: saveFuncReport, onClose: popModal }))),
