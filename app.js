...
const fused = mergeById(localData[stateKey] || [], remote);
merged[stateKey] = fused;
...
/* ── FIRME ── */
.sign-row { display: flex; gap: 40px; margin-top: 32px; break-inside: avoid; page-break-inside: avoid; }
...
.sign-row { display:flex; gap:32px; margin-top:24px; break-inside:avoid; page-break-inside:avoid; }
.sign { flex:1; text-align:center; break-inside:avoid; page-break-inside:avoid; }
.sign .line { border-top:1px solid #94a3b8; margin-top:8px; padding-top:5px; }
.sign .label { font-size:9px; color:#64748b; }
</style></head><body><div class="wrap"><div class="side"></div><div class="main">
<div class="header">
<div>
...
try {
const { merged, results } = yield supabaseSyncMerge(data);
onCloudPull(merged);
...
return;
if (remote.assets)
setAssets(prev => mergeById(prev, remote.assets));
if (remote.parts)
setParts(prev => mergeById(prev, remote.parts));
if (remote.jobs)
setJobs(prev => mergeById(prev, remote.jobs));
if (remote.orders)
setOrders(prev => mergeById(prev, remote.orders));
if (remote.withdrawals)
setWDs(prev => mergeById(prev, remote.withdrawals));
if (remote.customers)
setCustomers(prev => mergeById(prev, remote.customers));
if (remote.invoices)
setInvoices(prev => mergeById(prev, remote.invoices));
if (remote.quotes)
setQuotes(prev => mergeById(prev, remote.quotes));
if (remote.instruments)
setInstruments(prev => mergeById(prev, remote.instruments));
if (remote.procedures)
setProcedures(prev => mergeById(prev, remote.procedures));
if (remote.iecReports)
setIecReports(prev => mergeById(prev, remote.iecReports));
if (remote.funcReports)
setFuncReports(prev => mergeById(prev, remote.funcReports));
showToast("☁ Dati uniti dal cloud (nessun dato locale perso)");
};
