...
const GROUP = new Set(["Inp","Sel","Txt","Grid","Span2","Modal","AlertChip","KpiCard","BarChart","Donut","AreaTrend","ExcelTable","ConfirmDialog","PromptDialog","StorageGauge","SettingsSection","TestInstrumentsHint"]);
const src = fs.readFileSync("src/app.js", "utf8");
const ast = parse(src, { ecmaVersion: "latest", sourceType: "module" });
const spans = [];
for (const n of ast.body) if (n.type === "FunctionDeclaration" && n.id && GROUP.has(n.id.name)) spans.push({ s: n.start, e: n.end, name: n.id.name });
