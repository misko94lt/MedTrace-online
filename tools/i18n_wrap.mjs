...
for (const file of process.argv.slice(2)) {
  ...
  if (src.includes('from "../constants/i18n.js"')) { console.log(file + ": già convertito, salto"); continue; }
  ...
  (function walk(n) {
    ...
    if (n.type === "Literal" && typeof n.value === "string" && looksUI(n.value)) {
      ...
      for (let i = chain.length - 2; i >= 0 && !ok; i--) {
        ...
        if (p.type === "CallExpression") { const cn = p.callee.name || (p.callee.property && p.callee.property.name); if (MSG_FNS.has(cn)) ok = true; break; }
