...
function FuncWizardForm({ initial, assetId: propAssetId, assets, customers, existingReports, templates, instruments, technicians, onSave, onClose, isAdmin, showToast, onClassic }) {
...
function FuncVerifyForm({ initial, assetId: propAssetId, assets, customers, existingReports, templates, instruments, technicians, onSave, onClose, isAdmin, showToast }) {
...
function iecGetMeasures(norm, cls, patientType, method, sfc, fixed) {
if (norm === "61010")
return [
...
function IecWizardForm({ initial, assetId: propAssetId, assets, customers, existingReports, instruments, technicians, onSave, onClose, isAdmin, onClassic }) {
const h = React.createElement;
const TEAL = "#2dd4bf", RED = "#ef4444";
...
function IECReportForm({ initial, assetId: propAssetId, assets, customers, existingReports, instruments, technicians, onSave, onClose, isAdmin }) {
const MEAS_TUTORIAL = {
encl: {
...
function cndToTemplate(cnd) { if (!cnd) return ""; var best = "", bl = 0; for (var i = 0; i < CND_TO_TPL.length; i++) { var p = CND_TO_TPL[i][0]; if (String(cnd).indexOf(p) === 0 && p.length > bl) { best = CND_TO_TPL[i][1]; bl = p.length; } } return best; }
...
"Come si usa: 1) scegli il REPARTO dal menu a tendina (es. Cardiologia); 2) premi 'Scansiona' col lettore RFID collegato, oppure 'Simula scansione' per provare senza hardware; 3) compare la lista degli apparecchi TROVATI, con le verifiche scadute o in scadenza evidenziate in rosso/ambra.",
...
function AreaTrend({ data, valueKey, color, height, suffix, money }) {
...
function Donut({ segments, size, thickness, centerTop, centerSub }) {
...
const funcRep = withCreateMeta({ id: "FV" + ts.slice(-9), reportNumber: getNextReportNumber(funcReports, "VF"), assetId: p.assetId, customerId: cid, templateId: p.templateId, date: today, technician: p.technician || "", sections: p.funcSections || {}, overallPass: !!p.funcPass, verifyStatus: "completata", verifyType: "periodica", nextDate: funcNext, notes: "", published: true });
const iecRep = withCreateMeta({ id: "R" + ts.slice(-7), reportNumber: getNextReportNumber(iecReports, "VSE"), assetId: p.assetId, customerId: cid, date: today, technician: p.technician || "", norm: p.iecConfig.norm, equipClass: p.iecConfig.equipClass, patientType: p.iecConfig.patientType, leakageMethod: p.iecConfig.leakageMethod, verifyType: "periodica", verifyStatus: "completata", measures: p.iecMeasures || [], visual: { housing: null, cable: null, connectors: null, labels: null, docs: null }, overallPass: !!p.vsePass, notes: "", published: true });
const ppm = withCreateMeta({ id: "PPM" + ts.slice(-6), reportNumber: getNextReportNumber(ppmReports, "PPM"), assetId: p.assetId, customerId: cid, date: today, technician: p.technician || "", items: p.maint || [], categoryId: p.categoryId, overallPass: !!p.overall, notes: p.notes || "", funcReportId: funcRep.id, iecReportId: iecRep.id, published: true });
...
React.createElement("div", { style: { fontSize: 11, color: "var(--text-2)", fontFamily: "'IBM Plex Mono', monospace", wordBreak: "break-all" } }, reconResult.unknown.join("  ")),
React.createElement("div", { style: { fontSize: 10, color: "var(--text-4)", marginTop: 4 } }, "Tag non associati a nessun apparecchio. Associali dalla scheda (campo EPC).")))),
