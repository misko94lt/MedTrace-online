...
React.createElement("input", { value: q, onChange: e => setQ(e.target.value), placeholder: __t("Cerca apparecchio (nome, marca, matricola)..."), style: { width: "100%", boxSizing: "border-box", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text)", padding: "9px 11px", fontSize: 13 } }),
...
React.createElement("div", { style: { fontSize: 11, color: "var(--text-3)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, (a.brand || "") + (a.model ? (" " + a.model) : "") + (a.assetCode ? (" \u00b7 " + a.assetCode) : ""))),
a.epc ? React.createElement("span", { style: { fontSize: 10, fontWeight: 800, color: "#f59e0b", background: "#f59e0b18", padding: "2px 6px", borderRadius: 4, whiteSpace: "nowrap" } }, __t("ha gi\u00e0 tag")) : null)),
!list.length && React.createElement("div", { style: { fontSize: 12, color: "var(--text-3)", fontStyle: "italic", padding: "6px 2px" } }, __t("Nessun risultato"))),
React.createElement("div", { style: { borderTop: "1px solid var(--border-2)", marginTop: 8, paddingTop: 8 } },
React.createElement("div", { style: { fontSize: 11, color: "var(--text-3)", fontWeight: 700, marginBottom: 5 } }, __t("Oppure segna come TAG-REPARTO (scansionarlo imposter\u00e0 il reparto):")),
React.createElement("div", { style: { display: "flex", gap: 6 } },
React.createElement("input", { value: wq, onChange: e => setWq(e.target.value), list: "rfid-ward-dl", placeholder: __t("Nome reparto (es. Cardiologia)"), style: { flex: 1, minWidth: 0, boxSizing: "border-box", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text)", padding: "9px 11px", fontSize: 13 } }),
React.createElement("datalist", { id: "rfid-ward-dl" }, (wards || []).map(w => React.createElement("option", { key: w, value: w }))),
React.createElement("button", { onClick: () => { const n = String(wq || "").trim(); if (n && onAssignWard) onAssignWard(n); }, disabled: !String(wq || "").trim(), style: { background: String(wq || "").trim() ? "#2dd4bf" : "var(--surface)", border: "none", borderRadius: 8, color: String(wq || "").trim() ? "#04201c" : "var(--text-4)", padding: "9px 12px", fontSize: 12, fontWeight: 800, cursor: String(wq || "").trim() ? "pointer" : "default", whiteSpace: "nowrap" } }, __t("Salva")))));
}
