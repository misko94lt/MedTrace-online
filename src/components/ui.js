...
React.createElement("button", { onClick: () => { setOptSearch(""); setOpenCol(open ? null : c.key); }, title: sel.length ? sel.join(", ") : __t("Filtra"), style: { display: "flex", alignItems: "center", gap: 4, width: "100%", background: sel.length ? "#2dd4bf18" : "var(--card)", border: "1px solid " + (sel.length ? "#2dd4bf66" : "var(--border-2)"), borderRadius: 4, padding: "3px 6px", color: sel.length ? "#5eead4" : "var(--text-2)", fontSize: 11, cursor: "pointer", outline: "none", whiteSpace: "nowrap", overflow: "hidden" } },
...
React.createElement("td", { colSpan: cols.length + (onEdit || onDelete || actions ? 1 : 0), style: { textAlign: "center", padding: 32, color: "var(--text-4)" } }, __t("Nessun risultato")))) : filtered.map((row, i) => {
...
if (valid.length < 2) {
return React.createElement("div", { style: { height: height + 16, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-4)", fontSize: 11 } }, __t("Dati insufficienti per il grafico"));
}
...
WebkitTapHighlightColor: 'transparent' } }, __t("Annulla")),
...
WebkitTapHighlightColor: 'transparent' } }, __t("Conferma"))))));
}
...
React.createElement("button", { onClick: () => answer(false), style: { background: 'var(--border-4)',
border: '1px solid #3A3A4A', borderRadius: 8, color: 'var(--text-2)', padding: '9px 18px',
cursor: 'pointer', fontSize: 13, fontWeight: 600, touchAction: 'manipulation' } }, __t("Annulla")),
...
React.createElement("span", null, errorList.length === 1 ? __t("Un campo da completare prima di salvare") : errorList.length + " campi da completare prima di salvare")),
...
"\u00B7 S/N: ",
selected.serial))),
React.createElement("button", { type: "button", onClick: (e) => { e.stopPropagation(); onChange(""); }, title: __t("Rimuovi selezione"), style: { background: "none", border: "none", color: "var(--text-3)", fontSize: 14, cursor: "pointer", padding: "2px 6px", touchAction: "manipulation" } }, "\u2715"),
...
React.createElement("span", { style: { color: "var(--text-3)", fontSize: 11 } }, "\u25BE"))))) : (React.createElement(React.Fragment, null,
React.createElement("input", { ref: inputRef, type: "text", value: query, onChange: e => setQuery(e.target.value), onKeyDown: handleKey, placeholder: __t("Cerca per codice, nome, marca, S/N, ubicazione\u2026"), style: Object.assign(Object.assign({}, INP_BASE), { cursor: "text" }) }),
...
} }, filtered.length === 0 ? (React.createElement("div", { style: { padding: "16px", textAlign: "center", color: "var(--text-3)", fontSize: 12 } }, assets.length === 0 ? __t("Nessun apparecchio registrato") : "Nessun risultato per \"" + query + "\"")) : (React.createElement(React.Fragment, null,
...
__t("Mostro primi 50 \u00B7 digita per cercare tra tutti i "),
assets.length)),
filtered.map((a, i) => {
var _a;
const cust = (_a = customers === null || customers === void 0 ? void 0 : customers.find(c => c.id === a.customerId)) === null || _a === void 0 ? void 0 : _a.name;
...
filtered.length === 100 && (React.createElement("div", { style: { padding: "6px 12px", fontSize: 10, color: "var(--text-3)", textAlign: "center", borderTop: "1px solid var(--border-2)", fontStyle: "italic" } }, __t("Solo primi 100 risultati \u00B7 affina la ricerca per vedere pi\u00F9 specifico"))))))))));
};
...
!hasContent && (React.createElement("div", { style: { position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none", color: "var(--text-2)", fontSize: 13, fontStyle: "italic" } }, __t("Firma qui (usa S-Pen o dito)")))),
React.createElement("div", { style: { display: "flex", justifyContent: "flex-end", gap: 6 } },
React.createElement("button", { type: "button", onClick: clear, style: { background: "transparent", border: "1px solid var(--border)", color: "var(--text-2)", borderRadius: 6, padding: "4px 10px", fontSize: 11, fontWeight: 600, cursor: "pointer", touchAction: "manipulation" } }, __t("Cancella firma")))));
};
