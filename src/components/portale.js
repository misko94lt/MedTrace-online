...
const revoca = () => {
appConfirm(t("Revocare l'accesso al portale per questo cliente? Non vedrà più i suoi apparecchi finché non lo riabiliti."), () => __awaiter(this, void 0, void 0, function* () {
setBusy(true);
setMsg("");
try {
const c = yield getSupabaseClient();
...
stato === "non_abilitato" && React.createElement(Btn, { sm: true, disabled: busy, onClick: abilita }, busy ? t("Abilito…") : "\uD83D\uDD11 Abilita portale"),
...
React.createElement(Btn, { sm: true, variant: "ghost", disabled: busy, onClick: revoca }, t("Revoca")))),
stato === "attivo" && React.createElement(Btn, { sm: true, variant: "ghost", disabled: busy, onClick: revoca }, busy ? "…" : t("Revoca accesso")))) : (React.createElement("div", { style: { fontSize: 11, color: "var(--text-3)", marginTop: 9 } }, t("Solo il superuser pu\u00F2 abilitare o revocare l'accesso al portale."))),
...
isSuperuser && stato === "non_abilitato" && (React.createElement("div", { style: { fontSize: 11, color: "var(--text-3)", marginTop: 8, lineHeight: 1.5 } }, t("User\u00E0 l'email dell'anagrafica qui sopra. Il cliente entrer\u00E0 nel portale con quella email (link di accesso): al primo ingresso viene collegato automaticamente.")))));
}
...
const doDelete = (r) => { appConfirm(t("Eliminare definitivamente questa richiesta? L'azione non è reversibile."), () => __awaiter(this, void 0, void 0, function* () { setBusyId(r.id); try {
...
React.createElement("div", { style: { fontSize: 20, fontWeight: 900, color: "var(--text)" } }, t("Richieste clienti")),
React.createElement("div", { style: { fontSize: 12, color: "var(--text-3)", marginTop: 2 } }, t("Richieste di intervento arrivate dal portale"))),
...
React.createElement("button", { onClick: onRefresh, title: t("Aggiorna"), style: { background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text-2)", borderRadius: 9, padding: "8px 13px", fontSize: 14, fontWeight: 700, cursor: "pointer", touchAction: "manipulation" } }, "\u21BB"))),
!online && React.createElement("div", { style: { padding: "12px 14px", background: "#f59e0b18", border: "1px solid #f59e0b55", borderRadius: 10, color: "#f59e0b", fontSize: 12, marginBottom: 14 } }, t("Le richieste arrivano dal cloud: serve la connessione e l'accesso.")),
rows.length === 0 ? (React.createElement("div", { style: { padding: "30px 20px", textAlign: "center", color: "var(--text-3)", fontSize: 13, background: "var(--bg-2)", border: "1px dashed var(--border-2)", borderRadius: 12 } }, filter === "attive" ? t("Nessuna richiesta da gestire.") : t("Nessuna richiesta."))) : (React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 12 } }, rows.map(r => {
...
r.status === "nuova" && React.createElement("button", { disabled: busy, onClick: () => setStatus(r, "presa_in_carico"), style: { background: "transparent", color: "#f59e0b", border: "1px solid #f59e0b55", borderRadius: 8, padding: "9px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer", touchAction: "manipulation" } }, t("Presa in carico")),
!done && React.createElement("button", { disabled: busy, onClick: () => setStatus(r, "chiusa"), style: { background: "transparent", color: "var(--text-2)", border: "1px solid var(--border)", borderRadius: 8, padding: "9px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer", touchAction: "manipulation" } }, t("Ignora")),
